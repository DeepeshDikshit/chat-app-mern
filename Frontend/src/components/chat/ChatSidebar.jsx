...
const ChatSidebar = ({ chats, activeChatId, onSelectChat, onNewChat, open }) => {
  ...
  const handleSaveEdit = async (e, chatId) => {
    e.stopPropagation();
    try {
      await api.put(`https://chat-app-mern-1-ijrt.onrender.com/api/chat/${chatId}`, { title: editTitle }, { withCredentials: true });
  ...
  const handleDelete = async (e, chatId) => {
    ...
    if (window.confirm('Are you sure you want to delete this chat?')) {
      try {
        await api.delete(`https://chat-app-mern-1-ijrt.onrender.com/api/chat/${chatId}`, { withCredentials: true });
        window.location.reload(); // Temporary solution - should use state management instead
      } catch (err) {
